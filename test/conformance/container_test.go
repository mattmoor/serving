...
func TestMustNotContainerConstraints(t *testing.T) {
	...
	testCases := []struct {
		...
		options: func(s *v1alpha1.Service) {
			s.Spec.RunLatest.Configuration.GetTemplate().Spec.DeprecatedContainer.Ports = []corev1.ContainerPort{{
				Name:          "arbitrary",
				ContainerPort: 8080,
			}}
		},
		...
		options: func(s *v1alpha1.Service) {
			...
			s.Spec.RunLatest.Configuration.GetTemplate().Spec.DeprecatedContainer.VolumeMounts = []corev1.VolumeMount{{
				Name:             "VolumeMount",
				MountPath:        "/",
		...
		options: func(s *v1alpha1.Service) {
			s.Spec.RunLatest.Configuration.GetTemplate().Spec.DeprecatedContainer.ReadinessProbe = &corev1.Probe{
		...
		options: func(s *v1alpha1.Service) {
			s.Spec.RunLatest.Configuration.GetTemplate().Spec.DeprecatedContainer.LivenessProbe = &corev1.Probe{
				Handler: corev1.Handler{
					HTTPGet: &corev1.HTTPGetAction{
						Path: "/",
						Port: intstr.FromInt(8888),
...
func TestShouldNotContainerConstraints(t *testing.T) {
	...
	testCases := []struct {
		...
		options: func(s *v1alpha1.Service) {
			...
			s.Spec.RunLatest.Configuration.GetTemplate().Spec.DeprecatedContainer.Lifecycle = &corev1.Lifecycle{
