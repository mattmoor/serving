...
func PatchServiceImage(t *testing.T, clients *Clients, svc *v1alpha1.Service, imagePath string) (*v1alpha1.Service, error) {
	...
	if svc.Spec.RunLatest != nil {
		newSvc.Spec.RunLatest.Configuration.GetTemplate().Spec.DeprecatedContainer.Image = imagePath
	} else if svc.Spec.Release != nil {
		newSvc.Spec.Release.Configuration.GetTemplate().Spec.DeprecatedContainer.Image = imagePath
	} else if svc.Spec.DeprecatedPinned != nil {
		newSvc.Spec.DeprecatedPinned.Configuration.GetTemplate().Spec.DeprecatedContainer.Image = imagePath
	} else {
