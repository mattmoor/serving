...
func TestBadConfigurationDownConvert(t *testing.T) {
	beta := &v1beta1.Configuration{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "asdf",
			Namespace:  "blah",
	...
	got := &Configuration{}
	if err := got.ConvertDown(context.Background(), beta); err == nil {
		t.Errorf("ConvertDown() = %#v, wanted error", got)
	} else if cce, ok := err.(*CannotConvertError); !ok || cce.Field != "containers" {
		t.Errorf("ConvertDown() = %#v, wanted bad field %q", got, "containers")
	}
}
