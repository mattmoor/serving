	...
	tests := []struct {
		...
		want: &apis.FieldError{
			...
			Paths: []string{"spec.manual", "spec.pinned", "spec.release",
				"spec.revisionTemplate", "spec.runLatest"},
		},
	}, {
		...
		s: &Service{
			...
			Spec: ServiceSpec{
				ConfigurationSpec: ConfigurationSpec{
					// TODO(mattmoor): Use template (or the name is wrong)
					RevisionTemplate: &RevisionTemplateSpec{
						Spec: RevisionSpec{
							RevisionSpec: v1beta1.RevisionSpec{
								PodSpec: v1beta1.PodSpec{
		...
		want: apis.ErrDisallowedFields("spec.revisionTemplate.spec.concurrencyModel"),
	}, {
