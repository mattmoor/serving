...
func (source *RevisionSpec) ConvertUp(ctx context.Context, sink *v1beta1.RevisionSpec) error {
	...
	ts := source.TimeoutSeconds
	sink.TimeoutSeconds = &ts
	sink.PodSpec = corev1.PodSpec{
		ServiceAccountName: source.ServiceAccountName,
		Containers:         []corev1.Container{source.Container},
		Volumes:            source.Volumes,
	}
	if source.BuildRef != nil {
		return ConvertErrorf("buildRef",
			"buildRef cannot be migrated forward, got: %#v", source.BuildRef)
	}
	return nil
}
...
func (source *RevisionStatus) ConvertUp(ctx context.Context, sink *v1beta1.RevisionStatus) {
	// TODO(mattmoor): Convert the Conditions?
...
func (sink *RevisionSpec) ConvertDown(ctx context.Context, source v1beta1.RevisionSpec) error {
	sink.ContainerConcurrency = RevisionContainerConcurrencyType(
		source.ContainerConcurrency)
	if source.TimeoutSeconds != nil {
		sink.TimeoutSeconds = *source.TimeoutSeconds
	}
	sink.ServiceAccountName = source.PodSpec.ServiceAccountName
	sink.Volumes = source.PodSpec.Volumes
	if len(source.PodSpec.Containers) != 1 {
		return ConvertErrorf("containers",
			"unexpected number of containers: %d",
			len(source.PodSpec.Containers))
	}
	sink.Container = source.PodSpec.Containers[0]
	return nil
}
