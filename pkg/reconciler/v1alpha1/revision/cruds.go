...
// This is a generic function used both for deployment of user code & autoscaler
func (c *Reconciler) checkAndUpdateKPA(ctx context.Context, rev *v1alpha1.Revision, kpa *kpav1alpha1.PodAutoscaler) (*kpav1alpha1.PodAutoscaler, Changed, error) {
	logger := logging.FromContext(ctx)

	desiredKPA := resources.MakeKPA(rev)
	// Preserve the serving state on the KPA (once it is the source of truth)
