...
func (c *Reconciler) reconcileDigest(ctx context.Context, rev *v1alpha1.Revision) error {
	...
	digest, err := c.resolver.Resolve(rev.GetContainer().Image, opt, cfgs.Controller.RegistriesSkippingTagResolving)
	if err != nil {
		rev.Status.MarkContainerMissing(v1alpha1.RevisionContainerMissingMessage(rev.GetContainer().Image, err.Error()))
		return err
	}
